...
// Enhanced Algorand service with comprehensive API integration
export class AlgorandAPIService {
  ...
  static async testConnectivity(): Promise<{ success: boolean; message: string; details?: any }> {
    try {
      ...
      // Test algod health
      const healthResponse = await fetch(`${MAINNET_NODE}/health`, {
        headers: API_TOKEN ? { 'X-Algo-API-Token': API_TOKEN } : {},
        mode: 'cors'
      });
